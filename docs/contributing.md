...
The default development branch is `dev-v5`.
Checkout the `dev-v5` branch:
...
As subject matter experts, *code owners* approve pull requests on the packages they own. There is a required minimum of one code owner for each package. *Code owners* are listed in [CODEOWNERS](https://github.com/microsoft/fluentui-blazor/blob/dev-v5/.github/CODEOWNERS).
...
* To become a *contributor*, a community member must have a pull request approved and merged into the project's `dev-v5` branch.
