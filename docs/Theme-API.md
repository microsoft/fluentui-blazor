...
This document describes the JavaScript/TypeScript theme helpers exposed by `src\Core.Scripts\src\Utilities\Theme.ts` (namespace `Microsoft.FluentUI.Blazor.Utilities.Theme`).
...
Also synchronizes the HTML `dir` attribute (`'ltr' | 'rtl'`) with stored settings. This affects DOM/layout only and does not change theme tokens.
