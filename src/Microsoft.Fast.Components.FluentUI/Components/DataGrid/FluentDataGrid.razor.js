export function init(gridElement) {
    ...
    const bodyClickHandler = event => {
        const columnOptionsElement = gridElement.querySelectorAll('.col-options');
...
export function checkColumnOptionsPosition(gridElement) {
    const colOptions = gridElement._rowItems[0] && gridElement.querySelectorAll('.col-options'); // Only match within *our* thead, not nested tables
    if (colOptions) {
...
function enableColumnResizing(gridElement) {
    gridElement.tHead.querySelectorAll('.col-width-draghandle').forEach(handle => {
