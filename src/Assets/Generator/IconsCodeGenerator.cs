...
internal partial class IconsCodeGenerator
{
    ...
    private static Dictionary<string, int> SizeAndContentIndexes { get; } = [];
    ...
    private static void AddProperties(StringBuilder builder, IEnumerable<Icon> icons, int indentation = 12)
    {
        ...
        foreach (var icon in icons)
        {
            if (SizeAndContentIndexes.TryGetValue(icon.Key, out var content))
            {
                builder.AppendLine($"{indentationString}public class {icon.Name} : Icon {{ public {icon.Name}() : base(\"{icon.Name}\", IconVariant.{icon.Variant}, {icon.Name}SizeAndContent.Values[{content}]) {{ }} }}");
            } else
            {
                var svgContent = icon.GetContent(removeSvgRoot: true).Replace("\"", "\\\"");
    ...
    /// <summary>
    /// Generates all sizes and content for all icons in they respective variant file.
    /// </summary>
    /// <param name="icons"></param>
    /// <returns></returns>
    public void GenerateCommon(IEnumerable<Icon> allIcons)
    {
        // Generate Icons static class 
        var builder = new StringBuilder();
        builder.AppendLine("// <auto-generated>");
        builder.AppendLine("//     This code was generated by a tool.");
        builder.AppendLine("//     Changes to this file may cause incorrect behavior and will be lost if");
        ...
        builder.AppendLine();
        builder.AppendLine("namespace " + Configuration.Namespace + ";");
        builder.AppendLine(); builder.AppendLine("public static partial class Icons");
        builder.AppendLine("{");
        builder.AppendLine("    /// <summary>");
        builder.AppendLine("    /// returns the tuple representing the size and content of the first smaller available size");
        builder.AppendLine("    /// </summary>");
        builder.AppendLine("    internal static (IconSize size, string content) GetValueForSize(this (IconSize size, string content)[] sizes, int size)");
        builder.AppendLine("    {");
        builder.AppendLine("        if (sizes.Length == 0) { new InvalidOperationException($\"{nameof(sizes)} must not be empty\"); }");
        builder.AppendLine("        if (sizes.Length == 1) { return sizes[0]; }");
        builder.AppendLine("        for (int i = sizes.Length - 1; i >= 0; i--)");
        builder.AppendLine("        {");
        builder.AppendLine("            if ((int)sizes[i].size <= size) return sizes[i];");
        builder.AppendLine("        }");
        builder.AppendLine("        return sizes[0];");
        builder.AppendLine("    }");
        builder.AppendLine("}");
        var iconsClassFile = new FileInfo(Path.Combine(Configuration.TargetFolder.FullName, $"Icons.cs"));
        File.WriteAllText(iconsClassFile.FullName, builder.ToString());

        var allVariants = allIcons.Select(i => i.Variant)
               .Distinct()
               .OrderBy(i => i);

        foreach (var variant in allVariants)
        {
            var icons = allIcons.Where(i => i.Variant == variant).OrderBy(i => i.Name);
            Logger.Invoke($"Generating {variant}, containing {icons.Count()} icons.");
            var variantContent = GenerateCommonForVariant(variant, icons);

            var file = new FileInfo(Path.Combine(Configuration.TargetFolder.FullName, $"{variant}.cs"));
            File.WriteAllText(file.FullName, variantContent);
        }
    }

    private string GenerateCommonForVariant(string variant, IEnumerable<Icon> icons)
    {
        var groupedIcons = icons
                .GroupBy(i => i.Name)
                .Select(i => new { Name = i.Key, Icons = i.OrderBy(i => i.Size).ToList() });
        ...
        {
            AddSizeAndContentClassForIconGroup(builder, group.Name, group.Icons, 8);
            builder.AppendLine($"        public class {group.Name} : Icon {{ public {group.Name}(int size) : base(\"{group.Name}\", IconVariant.{variant}, {group.Name}SizeAndContent.Values.GetValueForSize(size)) {{ OverriddenWidth = size; }} }}");
            builder.AppendLine();
        }
    ...
    private static void AddSizeAndContentClassForIconGroup(StringBuilder builder, string name, IEnumerable<Icon> icons, int indentation = 12)
    {
        string indent1 = new(' ', indentation);
        string indent2 = new(' ', indentation + 4);
        string indent3 = new(' ', indentation + 8);

        builder.AppendLine($"{indent1}internal static class {name}SizeAndContent");
        builder.AppendLine(indent1 + "{");
        builder.AppendLine($"{indent2}internal static (IconSize, string)[] Values = new[]");
        ...
        foreach (var icon in icons)
        {
            var svgContent = icon.GetContent(removeSvgRoot: true).Replace("\"", "\\\"");
            builder.AppendLine();
            builder.Append($"{indent3}(IconSize.Size{icon.Size}, \"{svgContent}\"),");
            SizeAndContentIndexes.Add(icon.Key, index++);
        }
        builder.Remove(builder.Length - 1, 1); // strip the last comma
        builder.AppendLine();
