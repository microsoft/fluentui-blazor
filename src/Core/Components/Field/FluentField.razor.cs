...
public partial class FluentField : FluentComponentBase, IFluentField
{
    ...
    private string? LabelStyle => new StyleBuilder()
        .AddStyle("width", Parameters.LabelWidth, when: () => !string.IsNullOrEmpty(Parameters.LabelWidth) && Parameters.LabelPosition != AspNetCore.Components.LabelPosition.Above)
        .Build();
