...
/// </summary>
public partial class FluentTree : FluentComponentBase, IDisposable
{
    private readonly Dictionary<string, FluentTreeItem> _allItems = new(StringComparer.OrdinalIgnoreCase);
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="FluentTree"/> class.
    /// </summary>
    ...
    {
        if (!_allItems.TryGetValue(args.AffectedId!, out var treeItem))
        {
            return;
        }
    ...
    /// <summary>
    /// Disposes the component and releases any resources it holds.
    /// </summary>
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    private static ITreeViewItem? FindItemById(IEnumerable<ITreeViewItem>? items, string? id)
    {
        if (items == null)
        {
            return null;
