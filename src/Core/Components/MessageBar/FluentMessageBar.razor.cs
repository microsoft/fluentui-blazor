...
public partial class FluentMessageBar : FluentComponentBase, IDisposable
{
    ...
    protected string? ClassValue => new CssBuilder(Class)
        ...
        //.AddClass("dark", () => GlobalDesign.Theme == FluentUITheme.Dark)
    ...
    /// </summary>
    [Parameter]
    public IMessageInstance Message { get; set; } = default!; //= MessageInstance.Empty();
    ...
    /// </summary>
    [Parameter]
    public EventCallback<MessageBarEventArgs> OnStateChange { get; set; }
    ...
    /// </summary>
    protected ActionLink<IMessageInstance>? Link => Message.Options.Link;

    /// <summary>
    /// Button to show as primary action.
    /// </summary>
    protected ActionButton<IMessageInstance>? PrimaryAction => Message.Options.PrimaryAction;

    /// <summary>
    /// Button to show as secondary action.
    /// </summary>
    protected ActionButton<IMessageInstance>? SecondaryAction => Message.Options.SecondaryAction;

    /// <summary />
    protected bool ShowPrimaryAction => !string.IsNullOrEmpty(Message.Options.PrimaryAction?.Text);

    /// <summary />
    protected bool ShowSecondaryAction => !string.IsNullOrEmpty(Message.Options.SecondaryAction?.Text);

    /// <summary />
    protected override void OnInitialized()
    {
        if (Message.Options.Icon != null && Message.Options.Intent == MessageIntent.Custom)
        {
            Icon = Message.Options.Icon;
        }
        else
        {
            Icon = Message.Options.Intent switch
            {
                MessageIntent.Info => new CoreIcons.Filled.Size20.Info(),
                MessageIntent.Warning => new CoreIcons.Filled.Size20.Warning(),
                MessageIntent.Error => new CoreIcons.Filled.Size20.DismissCircle(),
    ...
    {
        _color = Message.Options.Intent switch
        {
        ...
        if (Message.Options.Timeout.HasValue)
        {
            ...
            _countdownTimer = new CountdownTimer(Message.Options.Timeout.Value).OnElapsed(DismissClicked);
    ...
    protected Task LinkClickedAsync()
    {
        ...
        {
            return Link.OnClick.Invoke(Message);
        }
    ...
    protected Task PrimaryActionClickedAsync(MouseEventArgs e)
    {
        ...
        {
            return PrimaryAction.OnClick.Invoke(Message);
        }
    ...
    protected Task SecondaryActionClickedAsync(MouseEventArgs e)
    {
        ...
        {
            return SecondaryAction.OnClick.Invoke(Message);
        }
    ...
    /// <summary>
    /// Pause the timeout countdown counter.
    /// </summary>
    ...
    /// <summary>
    /// Resume the timeout countdown counter.
    /// </summary>
    ...
    /// <summary />
    private async Task<MessageBarEventArgs> RaiseOnStateChangeAsync(MessageBarEventArgs args)
    {
    ...
    /// <summary />
    internal Task<MessageBarEventArgs> RaiseOnStateChangeAsync(IMessageInstance instance, MessageBarState state) => RaiseOnStateChangeAsync(new MessageBarEventArgs(instance, state));
    ...
    public void Dispose()
    {
        ...
        //GlobalDesign.OnChange -= StateHasChanged;
    }
}
